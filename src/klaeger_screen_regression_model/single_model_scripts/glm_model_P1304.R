...
norm_recipe <- 
	recipe(
		viability ~ ., 
		data = cell_line_compound_splits$P1304$splits[[1]]$data
	) %>%
	step_center(all_predictors()) %>%
	step_scale(all_predictors()) %>%
	# estimate the means and standard deviations
	prep(training = cell_line_compound_splits$P1304$splits[[1]]$data, retain = TRUE)
...
) %>% write_rds(here('results/glm_regression_models/P1304.rds'), compress = 'gz')
toc()