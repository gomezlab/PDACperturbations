...
Just a quick version of the model where we exclude a single compound and then guess that the viability of the missing compound is simply the average of all the viability binary values.
...
for (this_cell_line in unique(klaeger_data_matches_full$cell_line)) {
	...
	for (exclude_compound in unique(klaeger_data_cell_line$drug)) {
		...
		these_prediction_results$predicted_viability = mean(as.logical(klaeger_train$viability_binary))
...
```{r below 90 1000 tree 10 mtry,fig.height=5,fig.width=12, echo=F}
null_line = data.frame(x1 = 0, x2 = 1, y1 = 0, y2 = 1)
...
```{r below 90 5000 trees,fig.height=5,fig.width=12, echo=F}
null_line = data.frame(x1 = 0, x2 = 1, y1 = 0, y2 = 1)
...
for (this_cell_line in unique(klaeger_data_matches_full$cell_line)) {
	...
	for (exclude_compound in unique(klaeger_data_cell_line$drug)) {
		...
		rf_mod <-
			rand_forest(trees = 5000, mtry = 10) %>%
			set_engine("ranger", num.threads = 12) %>%
			set_mode("classification")
...
```{r below 90 5000 trees 10 mtry,fig.height=5,fig.width=12, echo=F}
null_line = data.frame(x1 = 0, x2 = 1, y1 = 0, y2 = 1)
...
```{r below 90 20000 tree,fig.height=5,fig.width=12, echo=F}
null_line = data.frame(x1 = 0, x2 = 1, y1 = 0, y2 = 1)
...
if (file.exists(here('src/klaeger_synergy_binary_model/predictions_LOO_measure.rds'))) {
	prediction_results_LOO_measure = read_rds(here('src/klaeger_synergy_binary_model/predictions_LOO_measure.rds'))
} else {
	prediction_results_LOO_measure = c()
	for (this_cell_line in unique(klaeger_data_matches_full$cell_line)) {
		
		klaeger_data_cell_line = klaeger_data_matches_full %>%
			filter(cell_line == this_cell_line)
		
		for (exclude_row in 1:dim(klaeger_data_cell_line)[1]) {
			...
			prediction_results_LOO_measure = bind_rows(
				prediction_results_LOO_measure,
				these_prediction_results
			)
			
		}
	}
	write_rds(prediction_results_LOO_measure,here('src/klaeger_synergy_binary_model/predictions_LOO_measure.rds'))
}
toc()
```
...
```{r, include=F}
ROC_vals = bind_rows(
	gather_ROC_vals(prediction_results_LOO_measure %>% filter(cell_line == "CAF"), "CAF"),
	gather_ROC_vals(prediction_results_LOO_measure %>% filter(cell_line == "P1004"), "P1004"),
	gather_ROC_vals(prediction_results_LOO_measure %>% filter(cell_line == "P1304"), "P1304"),
) %>% rename(cell_line = prediction_type) %>%
	arrange(cell_line,desc(cutoff))
...
AUC_plot = ggplot(ROC_vals) + 
	...
	labs(x="FPR",y="TPR") +
	theme(legend.position = "none") +
	ggtitle("Below 90 LOO Measurement") +
	BerginskiRMisc::theme_berginski()
...
AUC_plot = ggplot(ROC_vals) + 
	...
	# geom_point(aes(x=FPR,y=TPR, color=cell_line),lwd=1) + 
	geom_line(aes(x=FPR,y=TPR, color=cell_line),lwd=1) +
	labs(x="FPR",y="TPR", color='Cell Line') +
	theme(legend.position = "none") +
...
if (file.exists(here('src/klaeger_synergy_binary_model/predictions_LOO_measure_below40.rds'))) {
	prediction_results_LOO_measure = read_rds(here('src/klaeger_synergy_binary_model/predictions_LOO_measure_below40.rds'))
} else {
	prediction_results_LOO_measure = c()
	for (this_cell_line in unique(klaeger_data_matches_full$cell_line)) {
	...
	write_rds(prediction_results_LOO_measure,here('src/klaeger_synergy_binary_model/predictions_LOO_measure_below40.rds'))
}
toc()
```
...
AUC_plot = ggplot(ROC_vals) + 
	...
	labs(x="FPR",y="TPR") +
	theme(legend.position = "none") +
	ggtitle("Below 40 LOO Measurement") +
	BerginskiRMisc::theme_berginski()
