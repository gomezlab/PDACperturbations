...
```{r}
klaeger_synergy_data = read_rds(here('results/klaeger_synergy_for_regression.rds'))

cell_viability_summary = klaeger_synergy_data %>%
	select(drug, concentration_M, viability, cell_line) %>%
	group_by(cell_line,concentration_M, drug) %>%
	summarise(mean_via = mean(viability))

klaeger_synergy_data_unique = klaeger_synergy_data %>% select(-cell_line,-viability) %>% unique()

klaeger_synergy_data_unique_tidy = klaeger_synergy_data_unique %>%
	pivot_longer(-c(drug, concentration_M), names_to = "gene", values_to = "relative_intensity")

num_affected_kinases = klaeger_synergy_data_unique_tidy %>%
	group_by(drug) %>%
	summarise(count = sum(relative_intensity != 1)) %>% 
	arrange(count)

num_kinase_hits = klaeger_synergy_data_unique_tidy %>%
	group_by(gene) %>%
	summarise(count = sum(relative_intensity != 1)) %>% 
	arrange(desc(count)) %>%
	mutate(X = 1:length(count))

klaeger_synergy_data_unique = klaeger_synergy_data_unique %>% 
	mutate(drug = fct_relevel(as.factor(drug), num_affected_kinases$drug)) %>% 
	arrange(drug, concentration_M) %>%
	mutate(Y = 1:length(drug))

klaeger_synergy_data_unique_tidy = klaeger_synergy_data_unique_tidy %>%
	left_join(num_kinase_hits %>% select(gene, X)) %>%
	left_join(klaeger_synergy_data_unique %>% select(drug, concentration_M,Y)) %>%
	mutate(relative_intensity = ifelse(relative_intensity > 2, 2, relative_intensity))
```

```{r}
heat_labels = klaeger_synergy_data_unique_tidy %>% group_by(drug) %>% summarise(mean_Y = mean(Y)) %>% mutate(X = -50, relative_intensity = 2)

klaeger_heat = ggplot(klaeger_synergy_data_unique_tidy, aes(x=X,y=Y,fill=relative_intensity)) +
	geom_tile() + 
	theme_void() +
