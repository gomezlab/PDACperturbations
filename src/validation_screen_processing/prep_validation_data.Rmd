...
```{r}
plate_row_DMSO_means_part2 = plate_vals_part2 %>%
	filter(compound == "DMSO") %>%
	group_by(row, plate_name) %>%
	summarise(DMSO_mean = mean(lum),
						num_DMSO = n())

plate_row_DMSO_means_part3 = plate_vals_part3 %>%
	filter(compound == "DMSO") %>%
	group_by(row, plate_name) %>%
	summarise(DMSO_mean = mean(lum),
						num_DMSO = n())

if (! all(plate_row_DMSO_means_part2$num_DMSO == max(plate_row_DMSO_means_part2$num_DMSO))) {
	stop("Number of DMSO values in a plate isn't correct.")
} else {
	plate_row_DMSO_means_part2 = plate_row_DMSO_means_part2 %>% select(-num_DMSO)
}

if (! all(plate_row_DMSO_means_part3$num_DMSO == max(plate_row_DMSO_means_part3$num_DMSO))) {
	stop("Number of DMSO values in a plate isn't correct.")
} else {
	plate_row_DMSO_means_part3 = plate_row_DMSO_means_part3 %>% select(-num_DMSO)
}
```
...
plate_vals_row_norm_part2 = plate_vals_part2 %>%
	left_join(plate_row_DMSO_means_part2) %>% 
	mutate(viability = lum/DMSO_mean*100) %>%
	qc_filter() %>%
	write_rds(here('results/validation_results/validation_screen_part2.rds'))

plate_vals_row_norm_part3 = plate_vals_part3 %>%
	left_join(plate_row_DMSO_means_part3) %>% 
	mutate(viability = lum/DMSO_mean*100) %>%
	qc_filter() %>%
	write_rds(here('results/validation_results/validation_screen_part3.rds'))

treatment_via_summaries_part2 = plate_vals_row_norm_part2 %>% 
	group_by(compound, concentration_M, cell_line) %>%
	summarise(sd = sd(viability),
						num_samples = n(),
						mean_via = mean(viability),
						CV = sd/mean_via)
treatment_via_summaries_part3 = plate_vals_row_norm_part3 %>% 
	group_by(compound, concentration_M, cell_line) %>%
	summarise(sd = sd(viability),
						num_samples = n(),
						mean_via = mean(viability),
...
```{r}
b = ggplot(plate_vals_row_norm_part2 %>% filter(compound != "DMSO"), aes(x=concentration_M, y = viability, color=qc_fail)) +
...
c = ggplot(plate_vals_row_norm_part3 %>% filter(compound != "DMSO"), aes(x=concentration_M, y = viability, color=qc_fail)) +
	geom_hline(aes(yintercept = 90),alpha=0.5) +
	geom_hline(aes(yintercept = 100),alpha=0.5, linetype = 2) +
	geom_hline(aes(yintercept = 120),alpha=0.5, linetype = 2) +
	scale_x_log10() +
...
ggsave(here('figures/validation_testing/validation_viability_part2_part3_row_norm.png'),width=20*0.75,height=15*0.75)
BerginskiRMisc::trimImage(here('figures/validation_testing/validation_viability_part2_part3_row_norm.png'))
```
